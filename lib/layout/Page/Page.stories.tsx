...
export const Default: Story = {
    play: async ({ canvas }) => {
        await expect(canvas.getByText('Test title').tagName).toBe('H1');
...
export const WithTitleAsH3: Story = {
    play: async ({ canvas }) => {
        await expect(canvas.getByText('This is H3 title').tagName).toBe('H3');
        await expect(canvas.getByText('Lorem ipsum dolor sit amet cons', { exact: false }).tagName).toBe('DIV');
    },
    args: {
        title: 'This is H3 title',
        titleType: 'h3',
        children: Default.args?.children
    }
};
